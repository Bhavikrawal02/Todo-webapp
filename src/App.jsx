...
function App() {
  ...
  return (
    <>
      <Navbar />
      <div className=" container bg-violet-300 mx-auto my-3 rounded-xl p-4 font-bold min-h-[90vh] lg:w-1/2 ">
        ...
        <div className="todos bg-violet-50 min-w-1/4">
          {todos.length === 0 && <div className="p-5">No todo to display</div>}
          {todos.map(item => (
            (finished || !item.iscompleted) && (
              <div key={item.id} className="todo flex p-1 min-w-1 justify-between items-center">
